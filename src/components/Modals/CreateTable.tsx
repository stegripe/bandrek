...
export default function CreateTableModal({
	...
	const [newTableData, setNewTableData] = useState<DatabaseDataType[]>([{ name: "", collation: "" }]);

	const handleInputChange = (index: number, event: ChangeEvent<HTMLInputElement>) => {
		const { name, value } = event.target;
		const list: DatabaseDataType[] = [...newTableData];
	...
	const handleAddClick = () => {
		setNewTableData([...newTableData, { name: "", collation: "" }]);
	};
	...
	const onFormSubmit = () => {
		...
		newTableData.forEach(table => {
			if (table.name === "" || table.collation === "") {
				setError("Please fill all fields");
				setLoading(false);
				return;
			}
			executeQuery(`create database ${table.name} collate ${table.collation}`, []).then(response => {
				if ("data" in response) {
					executeQuery("show databases", []).then(() => {
